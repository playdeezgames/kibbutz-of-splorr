...
let private makeLogEntries 
        ...
        : unit =
    [1..howMany]
    |> List.iter (fun _ -> DwellerHistoryStore.LogForDweller (Dummies.ValidDwellerIdentifier, 0UL, Line "Message"))
...
let ``GetHistoryPageCount.It returns 2 when there is a one more than a full page of data.``() =
    ...
    makeLogEntries 11
    let actual = DwellerHistoryStore.GetHistoryPageCount Dummies.ValidDwellerIdentifier
